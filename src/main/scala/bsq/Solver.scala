...
class Solver {
  def toIntMap(
      currentBoard: Option[Array[String]]
    ...
    if (currentBoard.isEmpty) None
      ...
      Some(
        currentBoard.get.map(line =>
          getIntArray(line, length).getOrElse(return None)
        )
      )
    }
  }
  ...
  def solve(currentBoard: Option[Array[String]]): Option[Array[String]] = {
        ...
        if (posOpt.isEmpty) bsq
          ...
          if (pos.isSquare(board)) {
            ...
            if (newBsq.size > bsq.size)
              getBiggestSquare(newBoard, pos.toNext(board), newBsq)
    ...
    def getSolvedMap(bsq: Square, oldBoard: Array[String]): Array[String] = {
      def addSolvedLine(idx: Int, sqSize: Int, line: String): String = {
        ...
        val endOfLine =
          if (sqSize + idx < line.length) line.slice(idx + sqSize, line.length)
          else ""
      ...
      for (idx <- oldBoard.indices)
        if (isLineWithSquare(bsq, idx))
          oldBoard(idx) =
            addSolvedLine(bsq.y - bsq.size + 1, bsq.size, oldBoard(idx))
      oldBoard
    }

    def isEdgeCase(board: Array[String]): Boolean =
    ...
    def findFirstDot(board: Option[Array[Array[Int]]]): Square = {
      ...
      def getFirstDot(board: Array[Array[Int]], pos: Square): Square = {
        def shouldGetToNextColumn(
            board: Array[Array[Int]],
            pos: Square
        ): Boolean =
          board.length == 1 && !pos.isOutOfBounds(
            board,
            Square(pos.x, pos.y + 1, 1)
          )

        def shouldGetToNextLine(
            board: Array[Array[Int]],
            pos: Square
        ): Boolean =
          board(0).length == 1 && !pos.isOutOfBounds(
            board,
            Square(pos.x + 1, pos.y, 1)
          )
        ...
        else if (shouldGetToNextColumn(board, pos))
          getFirstDot(board, Square(pos.x, pos.y + 1, 1))
        else if (shouldGetToNextLine(board, pos))
